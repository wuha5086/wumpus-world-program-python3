...
# Helper function that creates the 5x5 wumpus world
def init_maze(maze, size):
    ...
    # Set spots around wumpus to stench
    if wump_x - 1 >= 0:
    ...
    for pit_y in range(size):
        for pit_x in range(size):
            ...
            if test_int < 3 and maze[pit_y][pit_x].empty and pit_x != gold_x and pit_y != gold_y:
                ...
                # Set spots around wumpus to stentch
                if pit_x - 1 >= 0:
...
def path_to_victory(maze, agent, points, cells):
    ...
    agent.current_y = 0
    print()
    print("Agent, away from the wumpus and got the gold!")
    return (points, cells)
...
def main():
    ...
    print('------------------------')
    print()
    print('5X5 Wumpus World Simulator:')
    init_maze(maze, int(5))
